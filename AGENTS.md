...
### SVG 설계 원칙
- **글자 크기**: 모든 텍스트는 **9px 이상** 사용
- **겹침 방지**: 글자가 겹쳐서 안보이는 구조는 최대한 회피
- **화살표/선**: 그 의미가 명확히 전달되도록 그릴 것
- **테마 고려**: 모든 색상은 CSS custom property 사용 (테마 전환 시 자동 적응)
...
- 모든 문서는 **한국어**로 작성 (코드/기술 용어 제외)
  - **한자(漢字)**: 반드시 한국어 표현 사용 (예: "기록" instead of "写入", "사용 가능" instead of "可以使用")
  - **일본어**: 절대 사용 금지
  - 코드 내 주석도 한국어로 작성
  - SVG 다이어그램 텍스트도 한국어 우선
- 새로운 페이지 작성 전, 기존 페이지를 참고하여 일관된 용어 사용
